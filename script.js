/*An object containing 3 arrays that contain possible values to form a complete message. It also contains a
  method that generates a new random message each time it is called*/
const components = {
    quotes: [
    ...
    activity: [
        ...
        'go for a swim at a nearby swimming pool',
        'go to the arcade with friends',
        'go shopping for new clothes',
        'stay at home and play computer games',
        'go to an amusement park',
        'stay at home and learn a new cooking recipe',
        'learn to play the guitar'
    ],

    generateRandomMessage() {
        selectedQuote = this.quotes[Math.floor(Math.random() * this.quotes.length)];
        selectedFood = this.food[Math.floor(Math.random() * this.food.length)];
        selectedActivity = this.activity[Math.floor(Math.random() * this.activity.length)];

        console.log(`The quote for today is: ${selectedQuote}. You will have ${selectedFood} for dinner. For today's activity, you will ${selectedActivity}.`);
    }
};

components.generateRandomMessage();